...
### Notes
- 32-bit systems do not provide hardware support for 64-bit arithmetic (refer to ![](1.10%20Bonus:%20CPU%20Architectures.md)). Compiling code with data that is `64 bits` in size requires generating sequences of operations to perform the arithmetic in 32-bit chunks.
